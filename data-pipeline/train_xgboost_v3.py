"""
Dëkkal — XGBoost v3 — Pikine 3 zones SAR
Dataset : 30 samples 100% SAR labels
Author  : Babacar Ndao
"""
import pandas as pd
import numpy as np
...
df = pd.read_csv('dekkal_pikine_clean_dataset.csv')

FEATURES = [
    'elevation_m', 'slope_deg', 'clay_pct',
...
print(f"Dataset Pikine 3 zones SAR :")
print(f"  Samples  : {len(df)}")
print(f"  flood=1  : {y.sum()}")
print(f"  flood=0  : {(y==0).sum()}")
print(f"  Balance  : {y.mean():.1%}")
...
df['correct']     = (df['flood_pred'] == df[TARGET]).astype(int)

print(f"\nPrédictions par zone :")
for zone in df['zone'].unique():
    z = df[df['zone']==zone]
    print(f"\n  {zone} (accuracy={z['correct'].mean():.0%}) :")
...
joblib.dump(model,    'dekkal_xgboost_v3.pkl')
joblib.dump(FEATURES, 'dekkal_xgboost_features_v3.pkl')
print(f"\n✓ Modèle v3 → dekkal_xgboost_v3.pkl")
